...
class Linear(Module):
    def __init__(
        ...
        ### BEGIN YOUR SOLUTION
        self.weight = Parameter(init.kaiming_uniform(in_features, out_features, dtype="float32", requires_grad=True))
        self.bias = Parameter(init.kaiming_uniform(out_features, 1, dtype="float32", requires_grad=True).transpose()) if bias else None
        ### END YOUR SOLUTION
